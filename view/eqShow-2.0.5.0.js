	...
	function d(a, b) {
		...
		j.contain.on("mousedown touchstart", function(a) {
			...
			if (a.stopPropagation(), c) {
				...
				if (e < j.pLen || e > 5) return c = !1, e > 0 && "NS" == h && (false ? j.touchPos = 0 : eqxiu.prePage()), void(0 > e && "NS" == h && (false ? j.touchPos = j.pLen : eqxiu.nextPage()));
				if (e > 0) return;
...
function(a) {
	...
	var i = a.templateParser("jsonParser", function() {
		...
		function d(a, d, e) {
			...
			if (i) for (i = c(i), f = 0; f < i.length; f++) if (i[f].sceneId = a.sceneId, 3 == i[f].type) {
				var j = l[("" + i[f].type).charAt(0)](i[f], d, a);
	...
	i.addInterceptor(function(a, b, c) {
		...
		return b
	}), i.addComponent("3", function(a, b,z) {
		...
		if(!z.video){
			...
			//console.log(z.num);
			var c, d = document.createElement("video");
			if ("edit" == i.mode ? (c = b ? b.parent(".edit_wrapper") : $("#nr .edit_wrapper"), d.setAttribute("class", "wrapper-background"),d.setAttribute("autoplay", "autoplay"),d.setAttribute("muted", "true")) : "view" == i.mode && (c = b ? b.parent(".edit_wrapper") : $("#edit_area" + i.def.id).parent(".edit_wrapper"), d.setAttribute("id", "video-background"),d.setAttribute("x-webkit-airplay", "true"),d.setAttribute("webkit-playsinline", "true"), d.setAttribute("muted", "true"), d.setAttribute("autoplay", "autoplay"),d.setAttribute("src", z.video)), 2 != a.properties.croptype || "edit" == i.mode) {
				var e = "100%";
				2 == a.properties.croptype && (e = 486 * a.properties.pageLength + "px"), $(d).prependTo(c).css({
					width: "100%",
					//height:"640",
			...
			d.addEventListener("ended", function() {
				eqxiu.nextPage();
				setTimeout(function(){
				var p = $(".z-current").find(".m-img").attr("id").substring(4);
					if(p == z.num){
						$('#video-background').get(0).play();
						//console.log(p);
					}
				}, 1000);
				//eqxiu.pageScroll(parseInt(a.pageId)+1);
			});
...
var eqxiu = function() {
		...
		function b(a) {
			...
			if($('#video-background').get(0)){
				$('#video-background').get(0).onplay=$('#video-background').get(0).pause();
			}
			...
			setTimeout(function(){
					var p = $(".z-current").find(".m-img").attr("id").substring(4);
					var v = $(".z-current").find("video").get(0);
						if(v){
							$('#video-background').get(0).play();
...
function(a, b) {
	...
	function parsePage(a, b) {
		...
		for (var k = 1; k <= a.length; k++) {
			if(b.obj.video){
				var array = b.obj.video.split(",");
				a[parseInt(array[0])-1].video = array[1];
			}
